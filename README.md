...
| 5가지 화면 테마, 선택 자동 저장, 다른 탭과 동기화                     | 구현·검증 완료 |
...
| 실제 공식 API 인증·전체 수집, 로컬 PostgreSQL 연결                    | 검증 완료      |
...
상단 **테마** 버튼에서 클라우드(화이트·블루), 미드나잇(네이비·블루), 라벤더(다크·퍼플), 샌드(아이보리·브라운), 포레스트(기존 다크·그린)를 선택할 수 있습니다. 기본값은 클라우드이며 배경, 메뉴, 검색창, 카드, 안내창에 함께 적용됩니다. 선택은 이 브라우저에 저장되어 새로고침 후에도 유지됩니다.
...
### Windows 로컬 실제 데이터 실행

기본 주소는 [http://127.0.0.1:3001](http://127.0.0.1:3001)입니다. `.env.local`의 `PORT`로 변경할 수 있습니다.

이 PC에 API 설정과 로컬 DB가 준비되어 있으면 `start-livescope.cmd`를 실행하거나 아래 명령을 사용합니다.

```bash
npm run dev:live
```

이 명령은 로컬 PostgreSQL을 시작하고 마이그레이션을 적용한 뒤, 실제 모드의 웹 서버와 5분 간격 수집 작업을 함께 실행합니다. 창을 닫으면 웹 서버와 수집 작업이 종료됩니다. 재실행하면 저장된 방송 카탈로그를 이어서 사용합니다. 이 PC의 기존 Node 20 대신 사용할 수 있는 Node 24 런타임을 `start-livescope.cmd`가 자동으로 선택합니다.

로컬 PostgreSQL은 [공식 Windows 배포 안내](https://www.postgresql.org/download/windows/)에서 연결하는 [EDB 압축본](https://www.enterprisedb.com/download-postgresql-binaries)의 `pgsql` 폴더를 `.tools/pgsql`에 놓고 사용합니다. 데이터는 `.data/postgres`에 저장되며 `127.0.0.1:55432`에서만 접속받습니다. `.env.local`의 DB 주소는 `postgresql://livescope:<24자 이상 임의 비밀번호>@127.0.0.1:55432/livescope`로 설정합니다. 앱에는 슈퍼유저 권한이 없는 전용 DB 계정을 사용합니다. 인증키·데이터·실행 파일은 GitHub에 업로드하지 않습니다.
...
`sync:watch`는 별도로 운영하는 웹 서버와 함께 사용할 수 있는 수집 전용 명령입니다. 웹 화면의 새로고침은 저장된 목록만 읽으므로 실제 방송 상태를 계속 갱신하려면 수집 프로세스도 실행 중이어야 합니다.
...
`npm run agent`로 stdio 도구 서버를 실행하고 `npm run agent:verify`로 실제 프로토콜 호출을 확인합니다. 방송 검색·정확한 채널 상태·전체 현황의 3개 읽기 도구를 제공합니다. AI 호스트가 도구를 선택하고 서버가 등록된 함수로 연결하여 입력 검증과 DB 조회를 수행합니다. [호스트 설정과 검증 기록](docs/AGENT_SERVER.md)을 참고하세요.
...
채널 조회에서 누락된 ID는 `needs_review`로 기록하며 즉시 삭제하지 않습니다. 숨김 채널은 다음 동기화로 다시 공개되지 않습니다. CLI 동기화는 30초 서버 캐시 재검증 및 브라우저의 다음 목록 조회 후 화면에 반영됩니다.
...
실제 라이브 목록은 페이지를 읽는 동안 변할 수 있으므로 완전 순회도 한 시점의 절대적 스냅샷을 보장하지 않습니다. 운영 전 API 호출량과 상태 반영 시간을 측정해야 합니다. 예약 작업이 시작된 채 프로세스가 중단되면 잠금은 DB 연결 종료 때 해제되고, 다음 동기화가 이전 RUNNING 기록을 정리합니다. 서버 캐시는 30초마다 재검증하며 인스턴스별 조회 시점에 따라 지연이 달라질 수 있습니다.
